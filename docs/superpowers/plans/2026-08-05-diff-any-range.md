...
/* Предпосчитаны только соседние переходы и сводный. Любой другой диапазон
   должен считаться на месте и совпадать с прямым сравнением тех же
   снапшотов — иначе «выбрать любой диапазон» означало бы «увидеть пустую
   таблицу». */
...
test('переход, которого нет в предпосчитанных, считается по требованию',
  function () {
    var dom = load();
    var a = snap('os-9.1', JUL, { builds: [build('nginx', { version: '1.0' })] });
    var b = snap('os-9.2', AUG, { builds: [build('nginx', { version: '2.0' })] });
    var c = snap('os-9.3', '2026-09-01T00:00:00+03:00',
                 { builds: [build('nginx', { version: '3.0' })] });
    store.add([a, b, c], 'a.json');
    /* os-9.2 → os-9.3 предпосчитан как соседний, а os-9.1 → os-9.3 —
       сводный; берём os-9.1 → os-9.2, он тоже соседний. Нужен переход,
       которого в наборе нет: при четырёх снапшотах это 9.2 → 9.4. */
    var d = snap('os-9.4', '2026-10-01T00:00:00+03:00',
                 { builds: [build('nginx', { version: '4.0' })] });
    store.add([d], 'b.json');
    var got = dom.pairFor(1, 3);
    var want = diffmod.diffSnapshots(b, d, false);
    assert.strictEqual(got.rows.length, want.components.length);
    assert.strictEqual(got.rows[0].status, 'upgraded');
    assert.strictEqual(got.old, 'os-9.2');
    assert.strictEqual(got['new'], 'os-9.4');
  });
```

Заглушке нужен доступ к `pairFor`. Экспорт `ui.js` расширяется на шаге 3;
`dom.pairFor` — обёртка, которую добавляет шаг 3 к возвращаемому объекту
`load()` в тесте. Если такой обёртки нет, тест падает на `dom.pairFor is
not a function` — это и есть ожидаемое падение.

- [ ] **Step 2: Убедиться, что тест падает**

Run: `node --test tests/js/ui.test.js`
Expected: FAIL — `dom.pairFor is not a function`
...
Наконец, в конце `ui.js` расширить возвращаемый объект, чтобы тест мог
позвать расчёт напрямую:

```js
  return { applyData: applyData,
           /* наружу — только для тестов: расчёт диапазона проверяется
              сравнением с прямым вызовом diffSnapshots, а через разметку
              его не достать */
           pairFor: function (lo, hi) { return pairFor([lo, hi]); } };
```

В `tests/js/ui.test.js`, в функции `load`, вернуть это наружу — заменить
`return dom;` на:

```js
  var api = require(UI);
  dom.pairFor = api.pairFor;
  return dom;
```
...
отдаёт его же, но проверять это через разметку нечем, а сравнивать
идентичность объектов пришлось бы через тестовый экспорт, заведённый ради
одной проверки. Кэш проверяется косвенно — тестом Task 1, шаг 5, где без

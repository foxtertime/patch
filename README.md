...
метки не имеют, их показывает раскрытая строка. И `openssl` без владельца и
без времени сборки — колонке «владелец» тоже есть что показать пустой.

Шестой собран с другого koji-хаба, и страница об этом предупреждает
окошком: сравнивать сборки разных хабов обычно бессмысленно, но бывает и
наоборот — переезд, зеркало, — поэтому снапшот принимается, а не
отвергается. Заодно видно, что ссылка на сборку ведёт на хаб того
снапшота, из которого сборка приехала: в паре os-9.4 → os-9.5 левая
...
такая сборка одна на сотню, и попадается она позже, чем правят вёрстку.

Восьмой — тег величиной с настоящий, сто пять сборок. Сортировка меняет
...
В колонке «владелец» стоит koji-логин того, кто запустил сборку. Сортировка
по ней собирает сборки одного человека подряд.
...
Строка с веткой подписана по виду ссылки: «ветка», когда билд собран с
ветки, и «коммит», когда с коммита. Значение в ней тогда не имя ветки, а
хеш, и подписать его веткой значило бы соврать.
...
Первым рядом стоят две большие карточки — стороны перехода: сколько сборок
в теге было и сколько стало. Числа берутся у самих снапшотов, а не по
строкам таблицы: строка — это компонент перехода, и компонент, которого на
этой стороне ещё или уже нет, в ней всё равно стоит. Под числом — тег и
время сбора: тега мало, два прогона одного тега законны, и «было os-9.4 →
стало os-9.4» без времени сбора не сказало бы, какой из них какой. Фильтра
у этих карточек нет — сторона перехода не срез таблицы, а то, между чем
считали; насколько сторон стало больше или меньше, говорит подсказка.
...
Сводка стороны — такая же полная карточка сборки, как в «Состоянии»:
...
сборки и проблемы, на «Изменениях» — статус и что изменилось. У каждой

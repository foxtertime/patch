...
включается только кнопкой и никогда — адресом: присланная ссылка сама по
себе не должна вешать вкладку тому, кто её просто открыл, даже если в ней
лежит опасный шаблон. Ключ `re=1` в адресе — по-прежнему запись, а не
команда: страница пишет его, когда режим включён, но при открытии такой
ссылки он сам не включается, а лишь предлагается — под полем встаёт
приглашение нажать кнопку `.*`. Промолчать об этом ключе было бы так же
неверно, как и подвесить вкладку: ссылка с шаблоном открылась бы с
отжатой кнопкой, искала бы его буквально, нашла бы скорее всего ноль
совпадений — и ничего не объяснила бы почему. Старые ссылки с `q=` без
`re` по-прежнему работают так же, как и до появления регулярок.
...
Текущий вид — активная вкладка, выбранный
...
(адрес, буфер обмена, окно) и свои обработчики, навешенные один раз.
...
| `address.js` | владение адресной строкой: `location.hash`, когда его читать и когда писать |

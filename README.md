...
конце строки отрезок, ведущий в пустоту. Составом управляют из списка,
который открывает кнопка
«изменить»: у каждого снапшота там стрелки вверх и вниз и крестик, убирающий
его из цепочки. Состоявшаяся перестановка отключает автоматическую
...
больше никто не трогает. Убрать можно и последний снапшот — страница тогда
возвращается к зоне загрузки.
...
получит. Но в шапке страницы его не будет, а фильтр по нему из присланной
ссылки опознается только там, где такие строки видны: класса нет в списке
страницы, и единственной опорой остаются теги строк выбранного снапшота.

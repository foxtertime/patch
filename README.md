...
SPEC / CHANGELOG / FILES / other) и строит самодостаточный HTML-дашборд с
вкладкой «Состояние» по каждому тегу и вкладкой «Изменения», сравнивающей
теги между собой.
...
| `patch_classes` | нет | правила для CVE, SAST, DAST, COVERAGE, SPEC, CHANGELOG, FILES + `other: '.*'` | список правил `{name, pattern}` — регулярка по имени файла патча → класс |
...
`changelog`, `files`, `other` (класс патча, присутствует в билде хотя бы
один патч этого класса), `inherited` (билд

...
Колонка «собран» показывает дату и время (UTC); время набрано бледнее даты,
чтобы колонка читалась по датам, а секунды не мешали их сравнивать. В
раскрытии то же значение стоит целиком, с пометкой `UTC`.
...
`YYYY-MM-DD HH:MM:SS`, в UTC, как его отдаёт хаб; в местное время оно не
переводится, иначе один и тот же снапшот читался бы по-разному у разных
людей, а снапшотами обмениваются. Доли секунды и смещение зоны срезаются.
Снапшоты, собранные до появления времени, несут одну дату — дашборд с ними
работает как прежде, просто без часов. `tag_name` — koji-тег, в котором билд действительно

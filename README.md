...
именно в этом порядке они сравниваются попарно, и он же виден в шапке
(`os-9.1 → os-9.2`). Составом управляют из списка, который открывает кнопка
...
случай: они различимы, и в цепочке у таких тегов-двойников рядом стоит дата
...
Над таблицей стоит переключатель снапшотов — при одном загруженном это одна
кнопка, а не пустое место: строка выбора не появляется и не исчезает от
того, сколько файлов подгружено. Открывается вкладка на последнем снапшоте
цепочки — при обычном порядке это самый свежий из загруженных.
...
подпись рельса меняется с «цепочка» на «выберите второй конец», а
...
конец, при котором правый ещё есть где-то правее. Если в написанном
порядке ссылка не читается вовсе, порядок разворачивается по цепочке.

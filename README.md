...
где чьи блоки. У сборки с проблемой полоса красная — та же, что метит строку
и без раскрытия.
...
Само раскрытие «было / стало» собрано парами: шапка к шапке, сводка к
сводке, патчи к патчам, пакеты к пакетам — каждая пара стоит в своей строке
сетки и потому имеет общую высоту. Без этого списки пакетов, посчитанные
построчно, начинались бы на разной высоте: патчей слева три, справа пять — и
вся пара уезжала бы вниз на два ряда, а выравнивание пропадало бы впустую.
...
Клик по карточке-счётчику или по метке строки ставит фильтр; несколько
...
единственной опорой остаются метки строк выбранного снапшота.
...
Шесть файлов в `assets/css/`, каждый про свой участок страницы. Порядок
...
| `layout.css` | шапка, панель источников, экран загрузки, вкладки, панель управления, чипы, кнопка «наверх» |
...
Шестнадцать файлов в `assets/js/`. Порядок в `SCRIPTS` задан по
...
**Владельцы участков DOM** — у каждого свой узел и свои обработчики,
навешенные один раз.

| Файл | За что отвечает |
|---|---|
| `rail.js` | рельс: разметка, выбор снапшота и диапазона, перестановка |
| `files.js` | загрузка снапшотов файлами и сообщения о ней |
| `tips.js` | подсказки |

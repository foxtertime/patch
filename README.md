...
сравнение с веткой включено и ветка действительно ушла вперёд
(`commits_ahead` не ноль) — а сам каталог `PATCH` в её вершине при этом
прочитался без ошибки; в остальных случаях, включая отказ GitLab на любом
из этих шагов, список пуст. Отсутствие ghost-патчей при непустом
`commits_ahead` не означает, что ветка не менялась, — только то, что
каталог `PATCH` в ней не менялся. В счётчики строки, карточки классов и
сводку «Изменений» ghost-патчи не идут — это то, чего в билде нет, а не
то, что в нём есть.

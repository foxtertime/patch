...
В раскрытии строки, в блоке `koji`, перечислены **все** koji-теги билда.
Основной — тот, через который билд попал в этот снапшот, — набран жирным
цветом текста и подписан «прямой» или «унаследован»; остальные приглушены:
они лишь сообщают, где билд висит ещё. Поиск ищет и по ним, так что запрос
вида `os-9.2-candidate` найдёт все билды, стоящие в этом теге.

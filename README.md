...
`tests/fixtures/snapshot-os-9.1.json` и `tests/fixtures/snapshot-os-9.2.json` —
обычные файлы снапшотов, они подгружаются так же, как собранные вами.
...
Появляется, только когда на странице два и более снапшота. Есть
переключатель переходов: последовательные пары `T1→T2`, `T2→T3`, … и, если
снапшотов больше двух, ещё и сводный переход `T1→Tn`; открывается вкладка на
самом широком из них. Пары строятся по цепочке — по тому порядку снапшотов,
который виден в шапке и который можно переставить руками.
...
`tests/fixtures/rich-old.json` и `rich-new.json`. С ним значение в значение

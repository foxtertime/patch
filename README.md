...
Берите все восемь: каждый следующий показывает то, чего не показывают
...
хаба и зеркала бывают. Второе такое предупреждение — снапшоты двух видов
рядом: у одних `patches` сняты с коммита сборки, у других (собранных до
2.2.0 или с `--no-branch-check`) — с вершины ветки, и часть разницы патчей
между ними тогда след смены смысла поля, а не событие в репозитории (см.
...
где чьи блоки. У билда с проблемой полоса красная — та же, что метит строку
и без раскрытия.
...
бейдж «ветка +N» — N коммитов после точки, из которой собран билд. Под
списком патчей тогда же появляется приглушённая секция ghost-патчей: три
стороны различия между деревом коммита и вершиной ветки, каждая со своим
заголовком и счётчиком. В счётчик патчей строки, карточки классов и сводку
«Изменений» они не идут — подробнее в «Формате снапшота», у полей
`commits_ahead` и `ghost_patches`.
...
Первым рядом стоят четыре большие карточки — итоги перехода, тот же ряд,
что на «Состоянии» занимают итоги тега: **было** и **стало** — сколько
билдов в теге на каждом конце, **разница** — на сколько их стало больше
или меньше, **срок** — сколько времени прошло между сборами.
...
Фильтра у этих четырёх нет: итог перехода — не срез таблицы, а то, между
чем считали, и кнопка обещала бы клик, которому нечего делать.

Ряды карточек занимают строку целиком, и последняя строка тоже. Браузер
сам этого не делает: он набивает строку под завязку и про остаток не
думает, поэтому одиннадцать срезов при десяти влезающих давали строку из
одной карточки и пустоту за ней. Страница считает, на сколько строк
карточки делятся поровну — одиннадцать по шесть это шесть и пять, — и
задаёт ширину сама; на узком окне тем же правилом делятся и четыре больших
итога, ложась два и два вместо трёх и одного. Пересчитывается это при
...
| Ключ | Что в нём |
|---|---|
| `commit` | коммит, из которого билд действительно собран — не то, что человек ввёл в `ref`, а то, чем реально прошла сборка. `null`, если не определён (билд без источника, `ref_kind: "srpm"`, ошибка GitLab) |
...
| `branch_head` | вершина ветки на момент сбора. Заполняется, только когда билд собран с ветки и сравнение с ней включено (`--no-branch-check` не передан) |
...
ветки. Второе бывает у билдов без известного коммита: у собранных до этой
версии dashboard, у сборки из готового SRPM (там `patches` и вовсе пуст) и
у той горстки, где GitLab уже не знает коммита сборки (ветку форс-пушнули,
коммит собрали мусором) — тогда каталог читают с вершины ветки и предупреждают
об этом в `problems`. Страница ловит, когда рядом легли снапшоты обоих
видов — часть разницы патчей между ними тогда след смены смысла поля, а не
...
- `branch` — файл есть в ветке, но появился после коммита сборки: в билд не
  вошёл, «готово в ветке, не собрано»;
- `changed` — путь тот же, а содержимое в ветке другое: в билде лежит
  прежняя редакция файла, «переписано в ветке после сборки» — так виден и
  переписанный после сборки патч CVE;
- `build` — файл был на коммите сборки, а из ветки его убрали: в билде он
  есть, «убрано из ветки после сборки».
...
заполняется только когда билд собран с ветки, сравнение с ней включено и
ветка действительно ушла вперёд (`commits_ahead` не ноль); в остальных
случаях список пуст. В счётчики строки, карточки классов и сводку
«Изменений» ghost-патчи не идут — это то, чего в билде нет, а не то, что в
нём есть.

...
отказа словами браузера. Есть и обратная сторона: регулярка вроде
`(a+)+$` устроена так, что перебор вариантов растёт экспоненциально, и
способна подвесить вкладку — остановить её в браузере нечем, ни тайм-аута,
ни отмены не бывает. Смягчает это только то, что дашборд ищет по коротким
строкам — именам компонентов, путям патчей, NVR, а не по вольному тексту,
где такой шаблон опаснее всего. Режим уезжает в адрес ключом `re=1`;
ссылка без него значит «выключен», и старые ссылки с `q=` без `re`
работают так же, как и до появления регулярок.

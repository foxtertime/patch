...
(`os-9.1 → os-9.2`). Порядок можно поправить руками — в списке «изменить» у
каждого снапшота есть стрелки вверх и вниз. Состоявшаяся перестановка
отключает автоматическую сортировку до конца жизни страницы: раз человек
сказал, в каком порядке сравнивать, дальше подгруженные файлы дописываются в
конец цепочки и порядок больше никто не трогает.
...
возвращается к прежнему, а причина встаёт рядом с именем файла.
...
Если снапшотов несколько, над таблицей стоит переключатель; открывается
вкладка на последнем снапшоте цепочки — при обычном порядке это самый
свежий из загруженных.
...
Снапшот стоит в хеше именем — тегом и временем сбора
(`tag=os-9.2@2026-08-01T00:00:00+03:00`), переход — именами обоих концов:
номер после перестановки или удаления
показал бы другой снапшот, ничем не выдав подмены, а одного тега мало, когда
на странице два прогона одного тега. Короткая форма `tag=os-9.2` тоже
принимается — её пишут руками, и она выбирает самый свежий снапшот с таким
тегом. Фильтр из хеша, которого нет ни на одной карточке живых данных,
молча выбрасывается: иначе страница показывала бы пустую таблицу под фильтр,
который нечем снять.
...
Дашборду этот порядок нужен для карточек классов и меток строк, а взять его
больше неоткуда: конфига у страницы нет. Поле необязательное — снапшоты,
собранные до его появления, читаются по-прежнему, и классы для них выводятся
из самих патчей по алфавиту; выдумывать за конфиг порядок нельзя. Когда
снапшотов несколько, порядок задаёт первый, а остальные могут только
дописать в конец то, чего в нём не было.

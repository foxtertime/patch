...
вместе. В `rich-legacy.json` нет ни одного поля, появившегося в 2.2.0:
так выглядит файл, записанный прежними выпусками, и патчи в нём сняты с
вершины ветки. У `nginx` там стоит `CVE-2026-3011.patch` — как патч
билда, хотя в билд он не входил. В `rich-drift.json` тот же файл стоит
ghost-патчем стороны «готово в ветке, не собрано», и рядом видно всё
...
появилось. Положив рядом любые два из трёх, вы увидите и предупреждение
о билдах двух видов — оно и существует ради этой пары.
...
не собранным, вообще не находился бы по CVE), CVE-ID и именам RPM. Текущий
вид — активная вкладка, выбранный

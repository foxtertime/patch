  ...
  function create(deps) {
    ...
    const st = {
      ...
      /* Идёт ли содержимое поля в регулярное выражение. Не по вкладкам:
         ...
         Меняется только кнопкой — адрес его больше не включает, см.
         reAsked ниже. */
      regex: false,
      /* Попросила ли присланная ссылка искать регулярным выражением. Замер
         показал, что шаблон вроде «(a+)+$» способен подвесить вкладку на
         десятки секунд — включать режим самим, в обход кнопки, опасно, а
         молча игнорировать ключ нельзя: ссылка с шаблоном открылась бы с
         отжатой кнопкой, нашла бы ноль по буквальному совпадению, и никто
         не сказал бы почему. Кнопка гасит признак: предложение принято или
         отвергнуто, и держать его дальше незачем. */
      reAsked: false,
    ...
    function restore(parsed) {
      ...
      /* Режим больше не переключается адресом: чужой шаблон способен
         подвесить вкладку ещё до того, как человек хоть что-то нажал, а
         снапшоты в этот момент уже могут быть в памяти. Включает его
         только кнопка. Но и молчать об этом ключе нельзя — иначе ссылка
         с шаблоном откроется с отжатой кнопкой, найдёт по нему буквальные
         совпадения (скорее всего — ноль) и ничем не объяснит почему.
         Поэтому запоминаем именно просьбу, а не применяем её; ставим
         всегда, а не только когда ключ есть, — молчание ссылки о режиме
         значит «не просила». Ровно '1', а не «ключ есть»: re=0 и пустой
         re= не должны читаться как включено. */
      st.reAsked = parsed.re === '1';
    }

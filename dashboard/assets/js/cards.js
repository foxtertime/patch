...
(function (root, factory) {
  ...
  /* Итоги перехода: сколько сборок было в теге, сколько стало, на сколько
  ...
  function pairCards(pair, from, to) {
    ...
    return side(from, was, 'было',
      ...
      + side(to, now, 'стало',
        `Сколько сборок в теге на конец перехода — все последние сборки `
        + `${to.tag} на момент того сбора.`)
      + cardHtml(null, true,
        (delta > 0 ? '+' : delta < 0 ? '−' : '') + size,
        plural(size, 'сборка', 'сборки', 'сборок'),
        `появилось ${pair.counts.added}, исчезло ${pair.counts.removed}`,
        'разница',
        'На сколько сборок в теге стало больше или меньше. Ноль здесь не '
      ...
      + cardHtml(null, true, gap[0] || '—', gap[1] || '',
        'между сборами', 'срок',
        'Сколько времени прошло между двумя сборами. Это не срок жизни '
        + 'тега: снапшот снимают когда придётся, и всё, что случилось с '

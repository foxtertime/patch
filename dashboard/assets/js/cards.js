...
(function (root, factory) {
  ...
  /* Тематический блок ряда: карточки внутри него отвечают на один вопрос.
     Ширину блока и его карточек задаёт css по этим же именам — сколько
     чего в строке, решает раскладка, а не скрипт.

     Блок, а не подпись над карточками: подписывать было бы нечего, каждая
     карточка названа сама, и второе имя над ними только повторяло бы их. */
  ...
  function stateCards(snap) {
    ...
    /* Три блока: сколько всего, что у билдов внутри и как прошёл сбор.
       В первом одна карточка на весь блок — число тега та точка отсчёта, к
       которой приводят остальные четыре, и стоять оно должно крупнее их. */
    const big =
      group('solo',
      ...
      /* Сбор — не про сам тег, а про то, что мы о нём знаем. У этих двух
         карточек свой предмет, и отделены они линией: подпись над блоком
         повторяла бы то, что на карточках и так написано. */
      + group('apart',
  ...
  function pairCards(pair, from, to) {
    ...
    /* Стороны — предмет перехода; разница и срок — про него же, но во
       вторую очередь. Блоки те же, что и на «Состоянии», и линия между ними
       та же: вкладки открывают одну за другой, и ряд, который при
  ...
  function diffCards(pair) {
    ...
    /* Две полосы: что стало с компонентами тега и что изменилось внутри
       них. Порядок карточек прежний — граница проходит ровно там, где один
